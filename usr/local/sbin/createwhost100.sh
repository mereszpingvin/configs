...
echo "Create FTP account..."
     FTPQ="use isabella
     INSERT INTO ftpquotalimits (name,quota_type,per_session,limit_type,bytes_in_avail,bytes_out_avail,bytes_xfer_avail,files_in_avail,files_out_avail,files_xfer_avail) VALUES ('$MYUSER', 'user', 'true', 'hard', '104857600', '0', '0', '0', '0', '0');
...
echo "Done"
echo "Sending mail..."
sed -e 's/azonosito/'$MYUSER'/g' -e 's/dmn/'$1'/g' -e 's/mysqlpass/'$MYSQLPW'/g' -e 's/ftppasswd/'$MYFTPW'/g' /usr/local/sbin/news100.tmpl > /usr/local/sbin/content100.html
(
...
echo "Subject: $MYUSER azonositoju 100MB-os tarhely elkeszult";
cat /usr/local/sbin/content100.html;
) | /usr/sbin/sendmail -t
rm -f /usr/local/sbin/content100.html
echo "Done"